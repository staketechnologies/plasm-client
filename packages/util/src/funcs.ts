...
export async function genUtxo(api: any, utxo: any) {
  const tx = await api.query.utxo.txList(utxo[0]);
  return new (Tuple.with([Transaction, u32]))([tx.get("payload"), utxo[1]])
}

export async function getUnfinalizeExitIdList(api: any) {
  return await api.query.parent.unfinalizedExits();
}